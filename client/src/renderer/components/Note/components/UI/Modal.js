...
const Modal = () => {
  ...
  return (
    <div
      ...
      } fixed inset-0 w-full h-full flex justify-center items-center z-30`}
    >
      <div
        onClick={() => hideModal()}
        className={`fixed w-full h-full left-0 top-0 ${
          modal ? 'animate-showModal' : 'animate-hideModal'
        } `}
      ></div>
      <div className="relative z-50 w-full px-4 sm:w-fit">{contentModal}</div>
    </div>
  );
};
