...
const EditNote = () => {
  ...
  useEffect(() => {
    if (noteId) {
      ...
      onAuthStateChanged(auth, (user) => {
        getDoc(doc(db, 'users', user.uid, 'notes', noteId))
          .then((res) => {
  ...
  const submitHandler = (e) => {
    ...
    onAuthStateChanged(auth, (user) => {
      const updateQuery = doc(db, 'users', user.uid, 'notes', noteId);
  ...
  return (
    <div className=" w-full h-96 sm:w-80 bg-white p-5 rounded-md animate-newNote">
      {loading === 'content-load' ? (
        ...
        <div className="flex flex-col justify-between h-full">
          <h3 className="mb-4">{editNoteForm.titleForm.text}</h3>
          <form
          ...
          >
              ...
              {editNoteForm.colorNote.map((item) => {
                return (
                  <div className="w-8 h-8" key={item.elementConfig.id}>
                    ...
                    />
                    <label
                      htmlFor={item.elementConfig.id}
                      className={`block w-8 h-8 rounded cursor-pointer ${colorInputHandler(
                        item.value
                      )}  peer-checked:ring-slate-600 peer-checked:ring-4`}
                    ></label>
                  </div>
                );
              })}
            ...
            <div className="flex justify-between space-x-4">
              <button
                ...
                className={`w-full px-4 py-1 border border-red-400 rounded-md transition duration-300 hover:border-transparent hover:bg-red-600 hover:text-white ${
                  loading === 'delete-submit'
                    ? 'disabled:bg-red-300 disabled:text-white cursor-not-allowed'
                    : null
                }`}
                disabled={loading}
              >
              ...
              <button
                type="submit"
                className={`w-full px-4 py-1 border border-blue-400 rounded-md transition duration-300 hover:border-transparent hover:bg-blue-600 hover:text-white ${
                  loading === 'edit-submit'
                    ? 'disabled:bg-blue-300 disabled:text-white cursor-not-allowed'
                    : null
                } `}
                disabled={loading}
              >
